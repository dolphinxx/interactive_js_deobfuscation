...
// Add the ability to exclude properties
...
export type Options = { props?: (string | symbol)[]; nonenumerable?: boolean, excludes?: (string | symbol)[]; }
...
export function copy<T>(target: T, options: Options = {}): T {
    ...
    return [...props, ...symbols].reduce<any>((carry, key) => {
        ...
        if(isArray(options.excludes) && options.excludes.includes(key)) {
            return carry;
        }
